# @manya-os/council Security Policy

## Scope
`@manya-os/council` runs entirely locally. No problem description, analysis content, debate transcript, minority opinion, consensus, or review report leaves the host process.
...
- **Specialist identity is not authenticated.** `specialistId` is a string; the council does not verify that the analyst submitting an analysis is who they claim to be. Authentication is the caller's responsibility (e.g. via `@manya-os/keyring`).
...
- **In-memory state.** `AnalysisCollector`, `DebateFacilitator`, `MinorityOpinionTracker`, and the `SpecialistRegistry` store their data in memory. Callers must persist their contents (e.g. via `@manya-os/ledger` or `serializeReport`) if they need durability.
