# @manya-os/constitution Security Policy

## Scope
`@manya-os/constitution` runs entirely locally. No governance context, ethical rule, policy condition, or audit-log entry leaves the host process.
...
## Known limitations
- **Audit log is in-memory.** It is not persisted to disk by default. Production deployments must call `getAuditLog()` periodically and persist the result to tamper-evident storage (e.g. `@manya-os/ledger`).
