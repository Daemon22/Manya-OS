...
export type RedactionStrategy =
  | 'mask'        // partial mask, e.g. `j***@gmail.com`
  | 'hash'        // deterministic hash, e.g. `[sha256:9f8a...]`
  | 'token'       // reversible pseudonym, e.g. `[NAME_001]`
  | 'redact'      // full replacement with `[REDACTED]`
  | 'generalize'  // coarse-grained, e.g. age → age band
  | 'synthesize'; // synthetic replacement preserving format
