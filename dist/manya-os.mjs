...
function issueCredential(params) {
  ...
  const id = params.id ?? "cred-" + randomUUID3();
...
var SoftwareKeyProvider = class {
  ...
  async generateKeyPair(algo, keyIdHint) {
    ...
    const keyId = keyIdHint ?? "sw-" + randomUUID4();
...
import * as crypto19 from "crypto";
...
var HashRedactor = class {
  ...
  strategy = "hash";
  redact(finding) {
    const h = createHash4("sha256").update(finding.text).digest("hex");
...
function dhash(input, width = 8, height = 8) {
  ...
  if (block === 0) return createHash5("sha256").update(input).digest("hex").slice(0, 16);
  const pixels = [];
...
function hashConfig(config) {
  ...
  return createHash6("sha256").update(canon).digest("hex");
}

// packages/anonymize/src/publishing/manifest.ts
import { createHash as createHash7 } from "crypto";
function hashRecord(record) {
  return createHash7("sha256").update(record).digest("hex");
}
function hashDataset(recordHashes) {
  return createHash7("sha256").update(recordHashes.join("\n")).digest("hex");
}
...
function randomId(prefix = "ctx", bytes = 8) {
  return `${prefix}_${randomBytes8(bytes).toString("hex")}`;
}
...
function createBackup2(snapshot) {
  ...
  const hash = createHash8("sha256").update(canon).digest("hex");
  return {
    schemaVersion: 1,
...
function newSubscriptionId() {
  subscriptionCounter++;
  try {
    if (typeof randomUUID9 === "function") return `sub-${randomUUID9()}`;
  } catch {
  }
