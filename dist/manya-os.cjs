...
__export(entry_exports, {
  ...
  CollaborationLedger: () => CollaborationLedger,
  CollaborationRequestQueue: () => CollaborationRequestQueue,
  ...
  validateCollaborationPackage: () => validateCollaborationPackage,
...
var Identity = class _Identity {
  ...
  metadata;
  constructor(params) {
    this.id = params.id ?? (0, import_crypto.randomUUID)(), this.did = params.did, this.publicKey = params.publicKey, this.algorithm = params.algorithm, this.createdAt = params.createdAt ?? (/* @__PURE__ */ new Date()).toISOString(), this.metadata = params.metadata ?? {};
  }
...
var CapabilityGrantManager = class {
  ...
  issue(params) {
    ...
    let now = (/* @__PURE__ */ new Date()).toISOString(), grant = { id: `grant_${(0, import_crypto3.randomBytes)(8).toString("hex")}`, grantor: params.grantor, grantee: params.grantee, resource: params.resource, actions: [...params.actions], validFrom: now, validUntil: params.validUntil, maxUses: params.maxUses, useCount: 0, revoked: false, metadata: params.metadata };
...
function issueCredential(params) {
  ...
  let id = params.id ?? "cred-" + (0, import_crypto4.randomUUID)(), issuedAt = params.issuedAt ?? (/* @__PURE__ */ new Date()).toISOString(), credential = { id, issuer: params.issuer, subject: params.subject, claims: params.claims, issuedAt, ...params.expiresAt !== void 0 ? { expiresAt: params.expiresAt } : {}, proof: { type: proofTypeFor(params.algorithm), created: issuedAt, verificationMethod: params.issuer, proofValue: "", algorithm: params.algorithm } }, bytes = canonicalCredentialBytes(credential), proofValue;
  try {
...
var SoftwareKeyProvider = class {
  ...
  async generateKeyPair(algo, keyIdHint) {
    let { publicKey, privateKey, algorithm } = generateKeyPair(algo), keyId = keyIdHint ?? "sw-" + (0, import_crypto5.randomUUID)();
  ...
  importExistingKey(publicKey, privateKey, algorithm, keyIdHint) {
    let keyId = keyIdHint ?? "sw-" + (0, import_crypto5.randomUUID)();
...
var crypto42 = __toESM(require("crypto"), 1);
var crypto52 = __toESM(require("crypto"), 1);
var fs22 = __toESM(require("fs"), 1);
var path2 = __toESM(require("path"), 1);
var child_process2 = __toESM(require("child_process"), 1);
var crypto62 = __toESM(require("crypto"), 1);
...
var crypto23 = __toESM(require("crypto"), 1);
var crypto33 = __toESM(require("crypto"), 1);
...
var HashRedactor = class {
  ...
  strategy = "hash";
  redact(finding) {
    let h = (0, import_crypto7.createHash)("sha256").update(finding.text).digest("hex");
...
function dhash(input, width = 8, height = 8) {
  ...
  if (block === 0) return (0, import_crypto8.createHash)("sha256").update(input).digest("hex").slice(0, 16);
  let pixels = [];
...
function hashConfig(config) {
  ...
  return (0, import_crypto9.createHash)("sha256").update(canon).digest("hex");
}
function hashRecord(record) {
  return (0, import_crypto10.createHash)("sha256").update(record).digest("hex");
}
function hashDataset(recordHashes) {
  return (0, import_crypto10.createHash)("sha256").update(recordHashes.join(`
`)).digest("hex");
}
...
function randomId2(prefix = "mem", bytes = 8) {
  return `${prefix}_${(0, import_crypto12.randomBytes)(bytes).toString("hex")}`;
}
...
function compress(payload) {
  ...
  let json = JSON.stringify(payload), originalBuf = Buffer.from(json, "utf8"), compressed = (0, import_zlib.gzipSync)(originalBuf);
...
function decompress(c) {
  ...
  let buf = Buffer.from(c.data, "base64"), json = (0, import_zlib.gunzipSync)(buf).toString("utf8");
...
function createBackup(snapshot) {
  ...
  let canon = JSON.stringify(snapshot), hash = (0, import_crypto13.createHash)("sha256").update(canon).digest("hex");
...
function verifyBackup(backup) {
  ...
  return (0, import_crypto13.createHash)("sha256").update(canon).digest("hex") === backup.hash;
}
...
var os22 = __toESM(require("node:os"), 1);
...
var ApplicationSource = class {
  ...
  constructor(opts) {
    let o = opts ?? {};
    this.id = o.id ?? "application", this.severity = o.severity ?? "info", this.tags = o.tags, o.emitter ? (this.emitter = o.emitter, this.ownsEmitter = false) : (this.emitter = new import_node_events2.EventEmitter(), this.emitter.setMaxListeners(0), this.ownsEmitter = true);
  }
  start(sink) {
